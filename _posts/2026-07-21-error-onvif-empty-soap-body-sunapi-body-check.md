...
같은 로그를 다시 볼 때는 아래 순서대로 확인하면 된다.
...
이 케이스에서 중요한 것은 `SOAP_NO_TAG`를 단순히 성공으로 무시하면 안 된다는 점이다.

요청이 실제로 적용됐을 수는 있지만, 응답 형식은 정상적이지 않다. 그래서 클라이언트 입장에서는 다음처럼 분리해서 처리하는 것이 안전하다.

| 상황 | 처리 방향 |
| --- | --- |
| 정상 `Response` 태그가 있음 | 정상 성공으로 처리 |
| `SOAP Fault`가 있음 | API 실패로 처리하고 Fault 내용 기록 |
| HTTP Body 자체가 없음 | 응답 누락으로 보고 재시도 또는 재조회 |
| HTTP Body는 있지만 SOAP Body가 비어 있음 | 응답 형식 불완전으로 기록하고 실제 적용 여부 확인 |
| 실제 적용도 확인되지 않음 | 실패로 보고 사용자에게 오류 안내 |
| 실제 적용은 확인됨 | 요청 처리 성공, 응답 파싱 실패로 분리 기록 |

즉, 이 오류는 “무조건 실패”도 아니고 “무조건 성공”도 아니다.

내가 판단한 기준은 다음에 가깝다.

```text
1. gSOAP 결과만 보면 실패다.
2. HTTP 통신과 인증은 성공한 것으로 보인다.
3. SOAP 응답 형식은 불완전하다.
4. 실제 적용 여부는 별도 검증으로만 판단할 수 있다.
5. 실제 적용이 확인되면 사용자 결과와 내부 로그를 분리해서 남긴다.
```

사용자에게는 변경이 적용되었는지가 중요하고, 개발자에게는 장비가 ONVIF 규격에 맞는 응답을 주지 않았다는 사실이 중요하다.

그래서 로그에는 다음을 남기는 것이 좋다.

```text
HTTP status: 200
Content-Type: application/soap+xml
Content-Length: 323
SOAP Envelope: exists
SOAP Body: empty
Expected response tag: missing
SOAP Fault: missing
Verification: changed value confirmed
Client result: response parse failed, operation result verified separately
```

이렇게 남기면 나중에 같은 장비나 같은 API에서 문제가 반복됐을 때, 인증 문제인지 응답 형식 문제인지 바로 구분할 수 있다.
